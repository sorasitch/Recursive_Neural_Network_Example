...
combined = concatenate([lstm_branch.output])
...
combined1 = concatenate([lstm_branch1.output])
z1 = Dense(64, activation="sigmoid")(combined1)
z1 = Dense(1, activation="linear")(z1)
# our model will accept the inputs of the two branches and
# then output a single value
model1 = Model(inputs=[lstm_branch1.input], outputs=z1)
model1.summary()

# combined2 = concatenate([lstm_branch2.output, model.output, model1.output,technical_indicators_branch2.output])
combined2 = concatenate([lstm_branch2.output])
z2 = Dense(64, activation="sigmoid")(combined2)
z2 = Dense(1, activation="linear")(z2)
# our model will accept the inputs of the two branches and
# then output a single value
model2 = Model(inputs=[lstm_branch2.input], outputs=z2)
model2.summary()

# combined3 = concatenate([lstm_branch3.output, model.output, model1.output, model2.output, technical_indicators_branch3.output])
combined3 = concatenate([lstm_branch3.output])
z3 = Dense(64, activation="sigmoid")(combined3)
z3 = Dense(1, activation="linear")(z3)
# our model will accept the inputs of the two branches and
# then output a single value
model3 = Model(inputs=[lstm_branch3.input], outputs=z3)
model3.summary()

# model

modelN = Model(inputs=[lstm_branch.input], outputs=[model.output])
modelN.summary()

modelN1 = Model(inputs=[lstm_branch1.input], outputs=[model1.output])
modelN1.summary()

modelN2 = Model(inputs=[lstm_branch2.input], outputs=[model2.output])
modelN2.summary()

modelN3 = Model(inputs=[lstm_branch3.input], outputs=[model3.output])
modelN3.summary()
...
history = modelN.fit([ohlcv_train], [y_train], batch_size=1024, epochs=100, shuffle=True, validation_split=0.1)
history1 = modelN1.fit([ohlcv_train], [y_train1], batch_size=1024, epochs=100, shuffle=True, validation_split=0.1)
history2 = modelN2.fit([ohlcv_train], [y_train2], batch_size=1024, epochs=100, shuffle=True, validation_split=0.1)
history3 = modelN3.fit([ohlcv_train], [y_train3], batch_size=1024, epochs=100, shuffle=True, validation_split=0.1)
...
y_predicted = modelN.predict([ohlcv_histories])
y_predicted1 = modelN1.predict([ohlcv_histories])
y_predicted2 = modelN2.predict([ohlcv_histories])
y_predicted3 = modelN3.predict([ohlcv_histories])
#y_predicted = model.predict([ohlcv_histories, technical_indicators])
