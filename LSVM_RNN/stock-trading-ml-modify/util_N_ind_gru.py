...
def csv_to_dataset(csv_path):
    ...
    ohlcv_histories_normalised = np.array([data_normalised[:,0:][i:i+(history_points*n):n].copy() for i in range(len(data_normalised) - End)])
    ...
    next_day_open_values_normalised = np.array([data_normalised[:, 0][i+1:i+(history_points*n)+1:n].copy() for i in range(len(data_normalised) - End)])
    ...
    next_day_open_values = np.array([data[:, 0][i+1:i+(history_points*n)+1:n].copy() for i in range(len(data) - End)])
    ...
    print("technical_indicators.shape = ", technical_indicators.shape)
